...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("cpacoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "cpacoin:");
    }
    if(!_request.startsWith("cpacoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with cpacoin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "cp2pSRsXYT3GQcDFN3VSnuSCLpDwfdrSMWm6BDnqgG1w51bDvgsqSGVFsu5xE3jTvReXVviqiqGkNE3Qym7EfHGo27hJmv6Kv";
