...
int main(int argc, char* argv[]) {
  ...
  QSettings cpacoinKey("HKEY_CLASSES_ROOT\\CpaCoinWallet", QSettings::NativeFormat);
  cpacoinKey.setValue(".", "Cpacoin Wallet");
  cpacoinKey.setValue("URL Protocol", "");
  QSettings cpacoinOpenKey("HKEY_CLASSES_ROOT\\cpacoin\\shell\\open\\command", QSettings::NativeFormat);
  cpacoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = CPA URL Handler\\nGenericName = cpacoin\\nComment = Handle URL Sheme cpacoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/cpacoin;\\nIcon = Cpacoin-Wallet' | tee /usr/share/applications/cpacoin-handler.desktop";
